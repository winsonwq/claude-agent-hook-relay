...
1. **说明问题**：清楚阐述需要决策的技术问题
2. **提出方案**：列出可能的选项（至少 2-3 个）
3. **等待确认**：等用户确认方案后再执行

**示例**：

```
❌ 错误做法：直接决定使用 Vitest，因为它是"现代、快速、TS 原生"

✅ 正确做法：
"推荐 Vitest，它相比 Jest 的优势是...，相比 Node 内置 test runner 的优势是...。
也可以考虑 Jest 或 Node 内置 test runner。你想用哪个？"
```

**需要等待确认的场景**：
- 测试框架选型
- 核心库的选择（如 HTTP client、logger 等）
- 架构层面的变更
- 任何影响项目方向的技术决策
